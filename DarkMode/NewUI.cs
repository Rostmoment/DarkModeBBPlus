...
namespace DarkMode
{
    ...
    {
        [HarmonyPatch(typeof(ModLoadingScreenManager), "Start")]
        [HarmonyPostfix]
        private static void FixLoadMenuScreen(ModLoadingScreenManager __instance)
        {
            Transform transform = GameObject.Find("NameEntry").transform.Find("New Game Object");
        ...
        private static void NewMenu(MainMenu __instance)
        {
            ...
            __instance.gameObject.transform.Find("Copyright").GetComponent<TextMeshProUGUI>().color = Color.white;
    ...
    public class NewUI : MonoBehaviour
    {
        ...
        void Start()
        {
            List<string> transforms = new List<string>() { "PickEndlessMap", "PickChallenge", "EndlessMapOverview", "About" };
            ...
            foreach (string s in transforms)
            {
                Transform t = LoadAsset<Transform>(s);
                try
                {
                    t.Find("BG").GetComponent<Image>().color = Color.black;
                }
